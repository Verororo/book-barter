...
const GivingOutSection = ({
  givingOutBooks,
  customizable = false,
}: GivingOutSectionProps) => {
  ...
  const handleDelete = async (deletedBookId: number) => {
    try {
      ...
      showNotification(
        'Succesfully deleted the book from the giving out section.',
        'success',
      );
      setLocalBooks((current) => current.filter((b) => b.id !== deletedBookId));
    } catch (error) {
      showNotification(
        'Failed to delete the book from the giving out section. Try again later.',
        'error',
      );
    }
  };
